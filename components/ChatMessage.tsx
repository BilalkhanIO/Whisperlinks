...
export const ChatMessage: React.FC<ChatMessageProps> = memo(({ message }) => {
  ...
  if (isSystem) {
    return (
      <div className="flex justify-center my-3 opacity-75 animate-in fade-in duration-300" role="status" aria-label={`System: ${message.text}`}>
  ...
  const senderLabel = isUser ? 'You' : (message.username ?? 'AI');

  return (
    <div
      className={`flex w-full mb-3 ${isUser ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-200`}
      role="article"
      aria-label={`${senderLabel} at ${timeLabel}`}
    >
      <div className={`max-w-[85%] sm:max-w-[70%] flex gap-2 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div
          ...
          aria-hidden="true"
        >
          {isUser ? <User size={14} /> : message.username === 'TruthBot' || message.username === 'Verifier' ? <BrainCircuit size={14} /> : <Globe size={14} />}
        </div>

        <div className={`flex flex-col min-w-0 ${isUser ? 'items-end' : 'items-start'}`}>
          {/* Username Label */}
          {!isUser && message.username && (
            <span className="text-[10px] text-zinc-500 ml-1 mb-0.5 font-mono tracking-wide uppercase truncate max-w-[150px]" aria-hidden="true">
              {message.username}
            </span>
          )}
          ...
          `}>
            {message.isEncrypted ? (
              <div className="font-mono text-neon-green text-xs tracking-widest opacity-80" aria-label={message.text}>
            ...
            <div className={`text-[9px] mt-1 opacity-40 flex gap-1 ${isUser ? 'justify-end' : 'justify-start'}`} aria-hidden="true">
              <span>{timeLabel}</span>
            </div>
          </div>
        </div>
