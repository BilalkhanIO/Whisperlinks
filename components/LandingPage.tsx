...
interface LandingPageProps {
  ...
  onEnter: (mode: ChatMode, roomConfig?: { name: string; code: string }) => void;
...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  // Detect join-mode from URL
  const searchParams = new URLSearchParams(window.location.search);
  const isJoining = !!searchParams.get('join');
  const joinRoomName = searchParams.get('room')
    ? decodeURIComponent(searchParams.get('room')!)
    : 'Private Room';

  // Room setup state (create flow)
  const [step, setStep] = useState<'main' | 'group-setup'>('main');
  const [localRoomName, setLocalRoomName] = useState('');
  const [enableCode, setEnableCode] = useState(false);
  const [localCode, setLocalCode] = useState('');

  // Keep room name in sync with username (if user hasn't edited it)
  const [roomNameEdited, setRoomNameEdited] = useState(false);
  useEffect(() => {
    if (!roomNameEdited) setLocalRoomName(`${username || 'User'}'s Room`);
  }, [username, roomNameEdited]);

  const handleRoomNameChange = (v: string) => {
    setLocalRoomName(v.slice(0, 32));
    setRoomNameEdited(true);
  };

  const handleCreateRoom = () => {
    onEnter('P2P', {
      name: localRoomName.trim() || `${username}'s Room`,
      code: enableCode ? localCode.trim() : '',
    });
  };

  const handleJoinRoom = () => onEnter('P2P');

  // If the user is joining via URL: show a simplified join card
  const renderJoinCard = () => (
    <div className="w-full bg-white/[0.025] backdrop-blur-2xl border border-white/[0.07] rounded-3xl p-8 shadow-2xl animate-slide-up">
      ...
      <div className="flex justify-center mb-6">
        <div className="relative animate-float">
          ...
          <div className="relative bg-void-dark p-5 rounded-2xl border border-neon-green/20 shadow-lg">
            <Users size={44} className="text-neon-green" aria-hidden="true" />
          </div>
        </div>
      </div>
      ...
      <div className="text-center mb-7 animate-slide-up delay-100">
        <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest mb-1">You're invited to</p>
        <h2 className="text-2xl font-black text-white leading-tight">{joinRoomName}</h2>
        <p className="text-xs text-zinc-500 mt-1.5 font-mono">Private · Encrypted · Zero Logs</p>
      </div>

      {/* Codename input */}
      <div className="relative mb-5 animate-slide-up delay-200">
        <input
          value={username}
          onChange={e => setUsername(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && username.trim() && handleJoinRoom()}
        ...
        />
        <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-mono ${username.length >= 10 ? 'text-yellow-500' : 'text-zinc-700'}`}>
          {username.length}/12
        </span>
      </div>
      ...
      <button
        ...
        className="w-full bg-neon-green text-black py-3.5 rounded-xl font-bold hover:bg-green-400 disabled:opacity-40 transition-all animate-slide-up delay-300 flex flex-col items-center gap-0.5"
      ...
      <p className="text-center text-[10px] text-zinc-600 mt-4 animate-slide-up delay-400">
        Room link expires when the host disconnects
      </p>
    </div>
  );

  // Room setup step card (create flow)
  const renderGroupSetupCard = () => (
    <div className="w-full bg-white/[0.025] backdrop-blur-2xl border border-white/[0.07] rounded-3xl p-8 shadow-2xl animate-in slide-in-from-right-4 duration-200">
      {/* Back */}
      <button
        onClick={() => { setStep('main'); setEnableCode(false); setLocalCode(''); setRoomNameEdited(false); }}
        className="flex items-center gap-1.5 text-zinc-500 hover:text-zinc-300 transition-colors text-xs mb-6"
      >
        <ArrowLeft size={13} />
        Back
      </button>
      ...
      {/* Title */}
      <div className="mb-6">
        <h3 className="font-bold text-zinc-100 text-xl leading-tight">Create Your Room</h3>
        <p className="text-xs text-zinc-500 mt-1">Set a name and optionally add a join code</p>
      </div>
      ...
      <div className="mb-4">
        ...
        <input
          ...
          className="w-full bg-zinc-900/80 border border-zinc-700/80 py-3 px-4 rounded-xl text-zinc-100 text-sm outline-none focus:border-neon-green/60 transition-all placeholder:text-zinc-600"
          aria-label="Room name"
        />
        <p className="text-[9px] text-zinc-600 font-mono mt-1 text-right">{localRoomName.length}/32</p>
      </div>
      ...
      <div className="flex items-center justify-between py-3 border-t border-b border-white/5 mb-4">
        <div>
          ...
          <p className="text-[11px] text-zinc-500 mt-0.5">Lock this room with a password</p>
        </div>
        <button
          onClick={() => { setEnableCode(e => !e); setLocalCode(''); }}
          className={`w-11 h-6 rounded-full transition-all relative shrink-0 ${enableCode ? 'bg-neon-green' : 'bg-zinc-700'}`}
          role="switch"
          aria-checked={enableCode}
          aria-label="Enable join code"
        >
          <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${enableCode ? 'translate-x-5' : 'translate-x-0'}`} />
        </button>
      </div>

      {/* Code input */}
      {enableCode && (
        <div className="mb-4 animate-in slide-in-from-top-2 duration-150">
          <label className="block text-[10px] font-mono text-zinc-500 uppercase tracking-wider mb-2">
            Join Code (share this privately)
          </label>
          <input
            value={localCode}
            onChange={e => setLocalCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8))}
            placeholder="E.G. SECRET42"
            className="w-full bg-zinc-900/80 border border-zinc-700/80 text-center py-3 rounded-xl text-zinc-100 font-mono tracking-[0.3em] text-base outline-none focus:border-neon-green/60 transition-all placeholder:text-zinc-600 placeholder:tracking-normal placeholder:text-sm"
            aria-label="Room join code"
            spellCheck={false}
          />
          <p className="text-[9px] text-zinc-600 mt-1.5">Share this code with guests out-of-band (e.g. via phone)</p>
        </div>
      )}
      ...
      <button
        ...
        disabled={!username.trim() || (enableCode && !localCode.trim())}
        className="w-full bg-neon-green text-black py-3.5 rounded-xl font-bold hover:bg-green-400 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
      >
        {enableCode && <Lock size={15} />}
        Create Room
      </button>
    </div>
  );

  // Main hero card (default landing)
    ...
    <div className="w-full bg-white/[0.025] backdrop-blur-2xl border border-white/[0.07] rounded-3xl p-8 shadow-2xl animate-slide-up">
      ...
      <div className="grid grid-cols-2 gap-3 animate-slide-up delay-300">
        <button
          onClick={() => onEnter('AI')}
          disabled={!username.trim()}
          className="group relative bg-zinc-100 text-black py-3.5 rounded-xl font-bold hover:bg-white disabled:opacity-40 transition-all flex flex-col items-center gap-0.5"
        ...
        <button
          onClick={() => { if (username.trim()) setStep('group-setup'); }}
          disabled={!username.trim()}
          ...
          aria-label="Create a group P2P room"
        >
        ...
        <section className="relative z-10 py-20 border-t border-white/5">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              />
              <FeatureCard
                icon={<Lock size={24} />} iconBg="bg-orange-500/10 text-orange-400"
                title="Room Code Lock"
                desc="Create password-protected rooms. Only guests with your secret code can enter."
              />
              <FeatureCard
        ...
        <section className="relative z-10 py-20 border-b border-white/5">
          <div className="max-w-3xl mx-auto px-6">
            ...
            <div className="space-y-0 divide-y divide-white/5">
              ...
              <FAQItem q="How does the room code work?" a="The host sets a code when creating the room and shares it privately. Guests must enter it to join — the code is never sent over the internet unencrypted." />
              <FAQItem q="Can I chat with multiple people?" a="Yes. Group Link creates a unique session. Share the link (and code if set) with trusted peers to open a group room." />
